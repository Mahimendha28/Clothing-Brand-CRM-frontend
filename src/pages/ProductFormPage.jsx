...
const buildAutoVariantSku = (productId, index) =>
  `SKU-${productId}-${Date.now()}-${index + 1}-${Math.floor(Math.random() * 1000)}`;
...
function ProductFormPage({ mode = "create" }) {
  ...
  const removeVariant = (index) => {
    ...
    if (removedVariant?.id) {
      setRemovedVariantIds((prev) =>
        prev.includes(removedVariant.id) ? prev : [...prev, removedVariant.id]
      );
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isEditMode) {
        // Keep product editing and variant inventory in sync without changing the existing APIs.
        for (let index = 0; index < validVariants.length; index += 1) {
          const variant = validVariants[index];
          const variantPayload = {
            sku: variant.sku || buildAutoVariantSku(nextProductId, index),
            size: variant.size,
            color: variant.color,
            price: variant.price,
            stock: variant.stock,
          ...
          if (variant.id) {
            ...
            await createProductVariant(nextProductId, variantPayload);
          }
        }
